...
uint16_t index = 0, sintable[400], costable[400];  // Initialize index, sine table and cosine table
uint16_t offset1 = 0, offset2 = 0;  // Initialize offset 1 and 2
...
int main(void)
{
    ...
    for (index = 0; index < 400; index++){
        ...
        costable[index] = (uint16_t)(1000.0*(1.0 + cos(6.28318531/400.*(float)index)));
    }
    index = 0;
...
interrupt void isr_timer0(void){
    ...
    DacaRegs.DACVALS.all = offset1 + sintable[index]; // Sine table output
    DacbRegs.DACVALS.all = offset2 + costable[index]; // Cosine table output
